...
# retourne la taille d'un tableau - nb de journaliste
def getSize(tab)
    return tab.length
end    

# Est ce que le tableau contient le caractère X ?
def tabContainsString(tab,words)
    ...
    tab.each { 
        |journalist|
        if (journalist.count words) > 0     # si on trouve au moins 1 occurence
...
# Est ce que le tableau contient ce mot ?
def tabContains(tab,word)
    ...
    tab.each { 
        |journalist|
        if (journalist.downcase).include? word.downcase # on met le handle en downcase et on recherche si le paramètre en downcase est présent
            compteur += 1
        end
}
    return compteur
...
# Combien de fois ce mot dans le tableau ?
def nbOccurence(tab,word)
    ...
    tab.each { 
        |journalist|    
            compteur += journalist.count word   # on compte le nombre d'occurences du mot dans tout le tableau
}
    return compteur
end
...
def orderbyAlpha(tab)
    return tab.sort_by{ |nom| nom.downcase }    # on trie par nom en les mettant en minuscules
    
end

# Combien de handles commencent par une majuscule ?
def charIsEqualTo(caractere,caractereToFind)
    return (caractere.count caractereToFind) > 0    # on cherche une occurence à l'endroit donné, du caractère donné
end

# Pseudos qui commencent par une majuscule
def beginByUpcase(tab)
    ...
    tab.each { 
        |journalist| 
        if charIsEqualTo(journalist[1],"ABCDEFGHIJKLMNOPQRSTUVWXYZ")    # en utilisant la fonction précédente, on regarde si le 2e caractère est une majuscule
            compteur += 1    
        end
    }
    return compteur
...
puts "Trie ta liste de handle par ordre alphabétique. Veux-tu afficher la liste ? (o/n)"
if gets.chomp == "o"
